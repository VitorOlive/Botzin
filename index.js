...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const NomerOwner = '5598988636013@s.whatsapp.net'
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
			   ...
			   break
			   case 'vitinnmenu':
		client.sendMessage(from, vitinnmenu(prefix, sender), text, {quoted: mek})
				break
				case 'menuadmin':
		...
		client.sendMessage(from, nsfwmenu(prefix, sender), text, {quoted: mek})
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Agora ele @${mentioned[0].split('@')[0]}\n é um membro comum kkkk _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'toin':
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Agora ele : @${mentioned[0].split('@')[0]}é um Adm,respeitem ele 😳`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
					case 'randomhentaio': 
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Vitinn", vcard: vcard}, MessageType.contact, { quoted: mek})
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 Vitinn Bot 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 Vitinn BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
